package com.codingfeel.sm.views.richeditor;
...
import com.codingfeel.sm.adapter.PostTagSpinnerAdapter;
...
public class RichEditTextEditor extends ScrollView {
    private static final int EDIT_PADDING = 15; // edittext常规padding是10dp
    ...
    private AppCompatSpinner topSpinner;
    private ImageView topImg;
    private TextView tvImg;
    private ArrayList<String> mPicUrls = new ArrayList<>();


    public RichEditTextEditor(Context context) {
        this(context, null);
    }

    public RichEditTextEditor(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RichEditTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
         ...
         * 初始化上面的布局
         */
        topView = inflater.inflate(R.layout.view_newpost_top, null);

        allLayout.addView(topView);
...
//        EditText firstEdit = createEditText("正文",
...
//            lastFocusEdit.setSelection(editStr1.length(), editStr1.length());
     ...
     * @param editStr EditText显示的文字
     */
    public void addEditTextAtIndex(final int index, String editStr, int totalSize) {


        if (allLayout.getChildCount() - 1 < totalSize) {
            int childViewSize = (totalSize - allLayout.getChildCount()) + 1;
            for (int i = 0; i < childViewSize; i++) {
                allLayout.addView(new View(getContext()));
            }
        }
    ...
    public void addImageViewAtIndex(final int index, Bitmap bmp,
        ...
        // 调整imageView的高度
        int imageHeight = (getWidth() - (int) getResources().getDimension(R.dimen.x64)) * bmp.getHeight() / bmp.getWidth();
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, imageHeight);
        lp.leftMargin = (int) getResources().getDimension(R.dimen.x32);
        lp.rightMargin = (int) getResources().getDimension(R.dimen.x32);
        lp.topMargin = (int) getResources().getDimension(R.dimen.x20);
     ...
     */
    public void addImageViewAtIndex(final int index, Bitmap bmp,
                                    String imagePath, int totalSize) {
        ...
        lp.leftMargin = (int) getResources().getDimension(R.dimen.x32);
        lp.rightMargin = (int) getResources().getDimension(R.dimen.x32);
        lp.topMargin = (int) getResources().getDimension(R.dimen.x10);
        imageView.setLayoutParams(lp);
     ...
     * 对外提供的接口, 生成编辑数据上传
     */
    public List<EditDataModel> buildEditData() {
        List<EditDataModel> dataList = new ArrayList<EditDataModel>();
        int num = allLayout.getChildCount();
        for (int index = 0; index < num; index++) {
            View itemView = allLayout.getChildAt(index);
            EditDataModel itemData = new EditDataModel();
    ...
    private void initTopView(final Context context) {
        topTitle = (EditText) topView.findViewById(R.id.et_newpost_title);
        topSpinner = (AppCompatSpinner) topView.findViewById(R.id.spinner_newpost_tags);
        topImg = (ImageView) topView.findViewById(R.id.iv_newpost_img);
        tvImg = (TextView) topView.findViewById(R.id.tv_newpost_img);
//
//        topImg.setOnClickListener(new OnClickListener() {
//            @Override
//            public void onClick(View view) {
//                MultiImageSelector selector = MultiImageUtil.getInstance().getMultiImageSelector(mPicUrls);
//                selector.start(context.getA, ConstantValue.REQUEST_IMAGE);
//            }
//        });

    }

    public void initImgListener(OnClickListener listener) {
        topImg.setOnClickListener(listener);
    }
    public void initTvImgListener(OnClickListener listener) {
        tvImg.setOnClickListener(listener);
    }
    public void initCoverListener(OnClickListener listener) {
//        topView.setOnClickListener(listener);
    }

    public void initTopSpinner(Context context, List<PostTagModel> postTagModels) {
        PostTagSpinnerAdapter adapter = new PostTagSpinnerAdapter(context, postTagModels);
    ...
    public void setTvImg(TextView tvImg) {
        this.tvImg = tvImg;
    }

    public EditText getTopTitle() {
        return topTitle;
    }

    public AppCompatSpinner getTopSpinner() {
        return topSpinner;
    }
